...
| `ChatGPT Image Aug 15, 2026, 11_07_24 PM.png` | **The logo, at source resolution** (1536×1024). Mark on the left, wordmark on the right, with a clean gap between them at x 785–828. Everything in `public/` is generated from this file — regenerate rather than hand-edit an icon. |
...
| (not in the handoff) | Plan a meal has two collapsed filter menus — Category and Tags — above the ingredient grid | **Mike, 2026-08-16**, asking for tag filtering "but I don't want the page to become an endless scroll of options". Right: the two vocabularies run past a dozen names, and flat chip rows would push the answer off the bottom of the phone. Closed they cost one 44px row and still say what is on; only labels something actually carries are offered. The recipe list keeps its flat chip row, where chips push a list down rather than hide the answer. |
| `Sheet` — bottom sheet for crop, confirm and the merge dialog (§6.8) | Built, and every confirmation in the app goes through it | **Mike, 2026-08-16**, asking for the pop-ups to be "stylized within app instead of within browser". The native `confirm()` was the only thing that didn't look like the app. Dismissing is always the safe answer, and `destructive: true` paints the button copper — neither of which a browser dialog can do. |
| Home-screen icon is a flat cream sprig on green | The logo's mark — book, vine and turning arrow — on `paper` | **Mike, 2026-08-16**, supplying the real logo. The flat sprig was a placeholder standing in until there was one. The wordmark is deliberately left out: it is unreadable below about 256px, so an icon carrying it is a smudge. The mark is trimmed to its own bounding box and squared on `paper`, which is also `background_color`, so the icon and the splash it generates are the same colour. |
| Settings has an API key field | It does, since phase 4 | The photo-parse that spends the key shipped, so the field it needs shipped with it. Still local storage only, still never in a backup. |
